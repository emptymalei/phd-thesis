...
\section{\label{chap:matter}Neutrino Oscillations in the Sun}


Flavor conversion occurs as long as their propagation eigenstates are different from their flavor eigenstates. As neutrinos travel from the center of the star through to the surface, we expect neutrino propagation eigenstates in such matter background to be different from flavor states in general~\cite{wolf78}. Using the fact that the neutral current interactions between the neutrinos and the matter is independent of the flavor, we only include the charged current interactions, which will produce an effective potential for electron flavor that is different from other flavors. In flavor basis, the effective potential is
\begin{equation}
