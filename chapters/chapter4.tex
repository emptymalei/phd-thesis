...
The conjecture between the flavor instablities and DR gaps does not work for the models with continuous ELN distribution $G(u)$ either. In Fig.~\ref{fig-box-c1} I show the dispersion relations for a step-like distribution
\begin{equation}
G(u) = \begin{cases}
-0.1 &  \text{if } -1 < u < -0.3, \\
1 &  \text{otherwise.}
...
I will consider the two-beam model depicted in Fig.~\ref{chap:collective-sec:two-beams-fig:two-beam-line-model} but with a ``neutrino mirror'' at $z=L$ that partially reflect the neutrino beams backward without changing their flavor content. The equations of motion for neutrino oscillations in this model are
\begin{align}
...
I developed a relaxation method to solve the two-beam model with reflection numerically. For the purpose of illustrating the algorithm, I denote the Hamiltonian using its components in flavor isospin space:
\begin{equation}
...
\end{algorithm}
To speed up the calculations, I have implemented OpenMP for parallel computing. 
...
As a first example, I calculated the neutrino oscillations with single neutrino beam only and propagating with emission angle $\theta=\pi/2$ (see Fig.~\ref{chap:halo-sec:line-fig:line-model-simplified}). Before I solving the numerical results, I will linearize the equation of motion and work out the solution for this single-beam line model.
...
In the linear regime, I define the density matrices for forward and backward beams to be
\begin{align*}
...
where
\begin{equation}
   \Delta = (1-R)^2 \mu^2  - 4\mu\xi \omega_\vv (1+R) + 4\omega_\vv^2.
\end{equation}
The corresponding eigenvectors are
\begin{align*}
   V_+ &=\begin{pmatrix}
   \frac{ -2\omega_\vv +  \mu (1+R) + \sqrt{\Delta} }{2\mu} \\
   1
   \end{pmatrix} \\
   V_- &=\begin{pmatrix}
   \frac{ -2\omega_\vv +  \mu (1+R) - \sqrt{\Delta} }{2\mu} \\
...
\begin{equation}
   ...
   \epsilon_{\mathrm F}(z) \\
   \epsilon_{\mathrm B}(z)
   \end{pmatrix} = C_- e^{-i\Omega_- L} \left( \frac{ \sqrt{\Delta} +  2\omega_\vv + \mu  (1-R) }{\sqrt{\Delta} -  2\omega_\vv - \mu (1-R)} V_+ e^{-i \Omega_+ (z-L)} +  V_- e^{-i \Omega_- (z-L)} \right).
\end{equation}

I am interested in exponential growth when $\Delta < 0$. The perturbation squared for the forward beam evolution is
\begin{align*}
   \left\vert \epsilon_{\mathrm F} \right\vert^2 \propto & \lvert (2\omega_\vv + \mu(1-R) +i \delta ) ( -2\omega_\vv + \mu(1+R) + i \delta ) e^{\delta(z-L)} \\
   &+ ( -2\omega_\vv - \mu(1-R) +i \delta ) ( -2\omega_\vv + \mu(1+R) - i \delta ) e^{-\delta(z-L)} \rvert^2,
\end{align*}
...
\end{align}
The only $z$ dependent term is $\cosh( 2\delta(L-z) )$, which is decreasing within $[0,L]$. The slope at $z=L$ is 0. 
